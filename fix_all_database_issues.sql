-- Comprehensive database fix for all missing functions and ambiguous column references

-- 1. Create get_current_member function
...
-- 2. Create get_current_cooperative function
...
-- 3. Create get_member_stats function
...
-- 4. Create get_cooperative_stats function
...
-- 5. Create get_cooperative_totals function (for backward compatibility)
...
-- 6. Update RLS policies to be more permissive for development
...
-- 10. Create a sample member for testing
INSERT INTO members (id, full_name, member_id, phone_number, status, cooperative_id, user_id)
SELECT 
  gen_random_uuid(),
  'Sample Member',
  'MEM-001',
  ...
  auth.uid()
FROM cooperatives c
WHERE c.contact_email = 'admin@example.com'
LIMIT 1
ON CONFLICT DO NOTHING;
