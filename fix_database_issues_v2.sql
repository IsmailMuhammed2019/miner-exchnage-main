-- Comprehensive database fix - Drop existing functions first, then create new ones
...
-- 10. Insert some sample data for testing (only if not exists)
...
-- 11. Create a sample member for testing (only if not exists)
...
AND NOT EXISTS (
  SELECT 1 FROM members m WHERE m.user_id = auth.uid()
)
LIMIT 1
